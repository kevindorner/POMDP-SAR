close all; clear all;
file2='SimpleCaseStudy.policy';
load('SimpleCaseStudy.mat');
...
for i=1:31 %Number of timesteps
...
XAxisMax=I(Nr_victims+1);

for x=1:Nr_victims
    sp(x)=subplot(2,Nr_victims,x); 
    pbaspect(sp(x),[1 1 1]); set(gca,'TickLength',[0, 0]);
    ...
    matrix= [0 1 1 5; 1 5 2 1; 3 0 1 2; 4 1 1 1; 5 6 3 1; 6 0 1 4; 7 0 1 2; 7 5 1 2; 0 7 2 1; 3 7 3 1];
    for i=1:length(matrix)
            rectangle('Position',[matrix(i,1)+0.5,matrix(i,2)+0.5,matrix(i,3),matrix(i,4)],'FaceColor','k','EdgeColor','k')
    end
    sp(x+Nr_victims)=subplot(2,Nr_victims,x+Nr_victims);
    pbaspect(sp(x+Nr_victims),[1 0.3 1]); set(gca,'TickLength',[0, 0])
    set(gca,'XTickLabel',[]);
    hold on; 
    yticks([round(reward(I(4)-1),2) round(reward(I(3)-1),2) round(max(reward),2) 100]);yticklabels({'','',round(max(reward),2),'V_{\pi}(b_t)'});
    xticks([XAxisMax]);xticklabels({'t'});
    axis([0 XAxisMax 0 100])
    %Plot arrowheads, labels
    text(XAxisMax-1.9,-0.1,'$\rightarrow$','Interpreter','latex','FontSize',8)
    text(-0.93,round(max(reward),2)*1.2-4,'$\uparrow$','Interpreter','latex','FontSize',8)
    text(0,round(max(reward),2)*1.2+20,'$V_{\pi}(b_t)$','Interpreter','latex','FontSize',8)
    ...
    if x<Nr_victims+1 
        ...
        if locdecode(i+1,1)==locdecode(i+1,2)
            if locdecode(i+1,1)==1
                ...
                subplot(2,Nr_victims,x+Nr_victims);
                yticks([round(reward(I(4)-1),2) round(reward(I(3)-1),2) round(max(reward),2) round(max(reward),2)*2]);yticklabels({'','',round(max(reward),0),'V_{\pi}(b_t)'});
...
for x=Nr_victims+1:2*Nr_victims
    subplot(2,Nr_victims,x);
...
subplot(2,3,3)
    plot([2 2],[4 5],'Color',[0.4660 0.6740 0.1880],'LineWidth',1)
    plot([2 3],[5 5],'Color',[0.4660 0.6740 0.1880],'LineWidth',1)
